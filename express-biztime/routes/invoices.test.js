...
describe("GET /1", function () {
    test("It retuns invoice info", async function () {
        ...
        expect(response.body).toEqual(
            {
                "invoice": {
                    id: 1,
                    amt: 100,
                    add_date: '2018-2018-01-01T08:00:00.000Z',
                    paid: false,
                    paid_date: null,
                    company: {
                        code: 'apple',
...
describe("POST /", function () {
    test("It shuould add invoice", async function() {
...
describe("PUT /", function () {
    ...
    test("It should retun 404 for no-such-invoice", async function (){
        const response = await request(app)
            .put("/invoices/9999")
            .send({amt: 1000});

        expect(response.status).toEEqual(404);
    });
    ...
    test("It should return 500 for missing data", async function () {
        ...
        expect(response.status).toEEqual(500);
    })
})
